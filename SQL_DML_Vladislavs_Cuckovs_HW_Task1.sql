...
2003, 1, 1, 4.99, 143, 29.99, 'PG-13'::mpaa_rating, CURRENT_DATE),
...
2001, 1, 3, 19.99, 178, 30.99, 'PG'::mpaa_rating, CURRENT_DATE),
( 'Grave of the Fireflies', 'A young boy and his little sister struggle to survive in Japan during World War II.',
1998, 1, 2, 9.99, 89, 49.99, 'PG'::mpaa_rating, CURRENT_DATE)) AS new_film (title, description, release_year, language_id,rental_duration,
									rental_rate, length, replacement_cost, rating, last_update)
WHERE NOT EXISTS (SELECT 1 FROM public.film f WHERE f.title  = new_film.title  AND    --CHECK IF NOT ALREADY EXISTS
					f.description = new_film.description AND f.release_year = new_film.release_year 
					AND f.language_id = new_film.language_id AND f.rental_duration = new_film.rental_duration
					AND f.rental_rate = new_film.rental_rate AND f.length = new_film.length
					AND f.replacement_cost = new_film.replacement_cost AND f.rating = new_film.rating
					AND f.last_update = new_film.last_update)
...
INSERT INTO public.film_actor (actor_id,film_id) --INSERT this information
SELECT * FROM( 
...
WHERE NOT EXISTS (SELECT 1 FROM public.film_actor fa WHERE fa.actor_id = new_film_actor.actor_id AND fa.film_id = new_film_actor.film_id) --CHECK IF NOT ALREADY EXISTS
RETURNING actor_id, film_id; --RETURN IF RECORDS ARE INSERTED
...
((SELECT film_id FROM public.film WHERE title ='The Lord of the Rings: The Fellowship of the Ring'),
   1, CURRENT_DATE),
((SELECT film_id FROM public.film WHERE title ='Grave of the Fireflies'),
   1, CURRENT_DATE)) AS new_inventory (film_id, store_id, last_update)
WHERE NOT EXISTS(SELECT 1 FROM public.inventory i WHERE i.film_id = new_inventory.film_id  --CHECK IF NOT ALREADY EXISTS
													...
													AND i.last_update = new_inventory.last_update)
